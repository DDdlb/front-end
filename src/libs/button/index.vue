...
const typeEnum = {
  ...
  info: 'text-zinc-800 bg-zinc-200 dark:bg-zinc-700 dark:text-zinc-300 hover:bg-zinc-300 dark:hover:bg-zinc-600 active:bg-zinc-200 dark:active:bg-zinc-700'
}
...
const props = defineProps({
  ...
  type: {
    type: String,
    default: 'main',
    // type 不在typeEnum中时给错误提示
    validator(val){
      const keys = Object.keys(typeEnum)
      const result = keys.includes(val)
      if(!result){
        throw new Error(`type 必须为${keys.join('、')}中的一个`)
      }
      return result
    }
  },
  ...
  size: {
      ...
      validator(val){
        ...
        const result = keys.includes(val)
        if(!result){
          throw new Error(`size 必须为${keys.join('、')}中的一个`)
        }
        return result
      }
  },
